            ...
            v(last_vertex).p, i};
               ...
               for (int i = 0; i < num_nodes; i++)
               {
                 ...
                 if (verbose)
                 {
                   ...
                   Point l = n->left - n_root;
                   Point r = n->right - n_root;
                   ...
                   debug.AddStartEvent(start, goal);

                   debug.AddGenerateEvent(n_root, n);
...
bool SearchInstance::search()
{
  ...
  while (!open_list.empty())
  {
    ...
    if (next_poly == end_polygon)
    {
      ...
      if (verbose)
      {
        debug.AddClosingEvent(node);
        debug.AddGenerateEvent(goal, true_final);
        debug.AddEndEvent(true_final, goal);
        debug.printToDebugFile();
    ...
    do
    {
      ...
      if (num_nodes == 1)
      {
        ...
        if (verbose)
        {
          ...
          // std::cerr << "test\n";
          //
          // const Point& n_root = (search_nodes_to_push[0].root == -1 ? start :
          //            mesh->mesh_vertices[search_nodes_to_push[0].root].p);
          //
          ...
          //
          // std::cerr << "root=" << root_to_point(n->root) << "; left=" << n->left
          //   << "; right=" << n->right << "; f=" << n->f << ", g="
          //   << n->g;

          // debug.AddGenerateEvent(root_to_point(search_nodes_to_push[0].root), &search_nodes_to_push[0]);
          //
          // std::cerr << "reached here as well\n";
        }
    #endif
      }
    }
